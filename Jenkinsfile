            ...
            steps {
                withCredentials([string(credentialsId: 'vault-token', variable: 'VAULT_TOKEN')]) {
                    script {
                        ...
                        echo '✅ Tous les secrets requis sont présents et validessss'
                    }
                }
            }
        }
            ...
            parallel {
                ...
                stage('Prepare Trivy Cache') {
                    steps {
                        retry(3) {
                            sh '''
                                docker volume create trivy-cache-frontend
                                docker run --rm \
        ...
        stage('Flux Reconciliation') {
            steps {
                sh '''
                    ...
                    flux reconcile kustomization frontend-service --timeout=3m || true
                    sleep 20
                    echo "📊 Pods:"
                    kubectl get pods -n microservices -l app=frontend-microservice || true
