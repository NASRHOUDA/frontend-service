pipeline {
    ...
    environment {
        DOCKER_IMAGE   = 'houdanasr/frontend-service'
        VAULT_ADDR     = 'http://host.docker.internal:8200'
        JENKINS_WS     = '/var/jenkins_home/workspace/frontend-service-pipeline'
        GH_USER        = 'NASRHOUDA'
    }
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                git branch: 'main',
                    url: 'https://github.com/NASRHOUDA/frontend-service.git',
            ...
            steps {
                withCredentials([string(credentialsId: 'vault-token', variable: 'VAULT_TOKEN')]) {
                    script {
                        ...
                        def required = ['DOCKER_USER','DOCKER_PASS','GH_TOKEN','SONAR_TOKEN']
        ...
        stage('Unit Tests') {
            steps {
                sh 'CI=true npm test -- --coverage --coverageReporters=lcov --passWithNoTests || echo "⚠️ Tests terminés"'
            }
        }
            ...
            steps {
                sh '''
                    docker run --rm \
                      ...
                      /var/jenkins_home/workspace/frontend-service-pipeline \
                      --no-git-ignore \
                      --json --output=/var/jenkins_home/workspace/frontend-service-pipeline/semgrep-report.json \
        ...
        stage('Prepare Trivy Cache') {
            steps {
                sh '''
                    mkdir -p /tmp/trivy-cache-frontend
                    docker run --rm \
                      -v /tmp/trivy-cache-frontend:/root/.cache/trivy \
        ...
        stage('Update Manifests') {
            steps {
                sh '''
                    ...
                    sed -i "s|image: houdanasr/frontend-service:.*|image: houdanasr/frontend-service:${BUILD_NUMBER}|g" kubernetes/deployment.yaml

                    git add kubernetes/deployment.yaml
                    if ! git commit -m "ci: update image tag to build #${BUILD_NUMBER} [skip ci]"; then
                        echo "⚠️ No changes to commit"
                    fi
                    git push "https://${GH_USER}:${GH_TOKEN}@github.com/NASRHOUDA/frontend-service.git" HEAD:main
        ...
        stage('Flux Reconciliation') {
            steps {
                sh '''
                    ...
                    sleep 20
                    echo "📊 Pods:"
                    kubectl get pods -n taskmanager -l app=frontend-microservice || true
    ...
    post {
        success { echo '✅ Pipeline frontend-service réussi !' }
